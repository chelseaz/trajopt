...
B = np.eye(d)
c = np.zeros((d, 1))
request["init_info"]["data_ext"] = np.r_[A.flatten(), B.flatten(), c.flatten()].tolist()

request["costs"][2]["X_s"] = [row.tolist() for row in x_nd.T]
request["costs"][2]["X_s_new"] = [row.tolist() for row in xtarg_nd.T]
request["costs"][2]["X_g"] = [row.tolist() for row in xtarg_nd.T]  ## TODO: correct this
request["costs"][2]["K"] = [row.tolist() for row in tps.tps_kernel_matrix(x_nd)]
